...
\begin{itemize}
\item For each pair of difficulty assessing methods the rank correlation was calculated between corresponding difficulty scores of objects from validation set.
\end{itemize}

Interpretability of Difficulty Assessment Methods:
\begin{itemize}
\item The various difficulty assessment methods were applied to objects from the primary dataset to evaluate the interpretability of the high difficulty scores obtained.
\end{itemize}

Connection between difficulty and consistency:
\begin{itemize}
\item In this experiment the connection between high model-specific difficulty and low consistency of objects was explored.
\item Firstly, for each difficulty assessing methods the rank correlation was calculated between difficulty scores and uncertainty scores of objects.
\item Secondly, we applied ROC-AUC to difficulty scores of validation set's objects, considering consistent objects as negative ones and inconsistent objects as positive ones.
\item Thirdly, the Kolmogorov-Smirnov test was used to check whether difficulty scores of consistent and inconsistent objects are from the same distribution or not.
\end{itemize}
