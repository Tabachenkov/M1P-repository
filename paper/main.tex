...
\begin{table}[t]
    \centering
    \caption{Considered articles and their correspondences to characteristics}\label{tab:system}
...
\begin{table}[t]
    \centering
    \caption{Considered functions of sklearn and their correspondences to characteristics}\label{tab:system2}
    \begin{tabular}{|p{6cm}|p{0.8cm}|p{0.8cm}|p{0.8cm}|p{0.8cm}|p{0.8cm}|p{0.6cm}|p{0.6cm}|}
...
$$IoU((begin_{x,1}, end_{x,1}, begin_{y,1}, end_{y,1}), (begin_{x,2}, end_{x,2}, begin_{y,2}, end_{y,2}) =$$$$= \frac{IntersectionArea}{Area_1 + Area_2 - IntersectionArea}$$
$$IntersectionArea =$$$$= (min(end_{x,1}, end_{x,2}) - max(begin_{x,1}, begin_{x,2}))(min(end_{y,1}, end_{y,2}) - max(begin_{y,1}, begin_{y,2}))$$
$$Area_1 = (end_{x,1}-begin_{x,1})(end_{y,1}-begin_{y,1})$$
$$Area_2 = (end_{x,2}-begin_{x,2})(end_{y,2}-begin_{y,2})$$

Subsequently, for each cluster, the authors computed an averaged bounding box, taking into account the reliability weights assigned to the assessors. For the computed bounding box and each class \( k \in \{1, \ldots, K\} \), the authors calculated a confidence score denoted by:
...
Let we have text $T = \{\tau_1, ..., \tau_l\}$, where $\tau_i$ is a token and its text span $s = \{\tau_{begin}, ..., \tau_{end}\}$ with the SpanTag $y \in Y$. Tokenization corresponds to the considered tokenizer. Thus, we consider triplet $(T, (begin, end), y)$.
...
Additionally, an arbitrary document may have a varying number of markups. To accommodate this, we adaptively adjust the parameters \(N\) and \(c_0\) in accordance with the increasing number of markups within a document, potentially considering the ratio of \(T\) to the total number of markups pertinent to the current document.
...
Further in the text, objects with high confidence scores (greater than threshold) will be called \emph{consistent}, objects will low confidence scores (less than threshold) will be called \emph{inconsistent}. In our experiments we chose 0.7 as threshold. Besides that, we introduce an \emph{uncertainty score} $UncerScore(\cdot) = 1-ConfScore(\cdot)$.
...
\begin{description}
    \item[Data Preprocessing] The datasets were transformed into a span-level data model, moving away from the chain-based structure.
    \item[Language Model Training and Validation] A single dataset was selected to serve as the primary reference. The model architecture was implemented according to the methodologies outlined in the vectorisation section, employing a span embedder coupled with a classifier at the output layer. Both training and validation procedures were conducted on this primary dataset, integrating the precomputed confidence scores from the assessors.
    \item[Validation of Difficulty Assessment Methods] The efficacy of the difficulty assessment methods was evaluated using methodologies aligned with those found in the established literature. Non-difficult objects were identified within the primary dataset, while the remaining datasets were used to source (pseudo)difficult objects. Furthermore, a technique that introduces noise to fragment embeddings was employed for additional assessment. The performance of the various methods was compared on this corpus of (pseudo)difficult data. Specifically, four distinct methods were validated: Variance of gradients~\cite{agarwal2022estimating}, Gaussian method~\cite{Lee2018ASU}, Margin-based approach, A variant of the margin-based approach utilising absolute values of margins.
    \item[Correlations between difficulty assessing methods] For each pair of difficulty assessing methods the rank correlation was calculated between corresponding difficulty scores of objects from validation set. 
    \item[Interpretability of Difficulty Assessment Methods] The various difficulty assessment methods were applied to objects from the primary dataset to evaluate the interpretability of the high difficulty scores obtained.
    \item[Connection between difficulty and consistency] In this experiment the connection between high model-specific difficulty and low consistency of objects was explored. Firstly, for each difficulty assessing methods the rank correlation was calculated between difficulty scores and uncertainty scores of objects. Secondly, we applied ROC-AUC to difficulty scores of validation set's objects, considering consistent objects as negative ones and inconsistent objects as positive ones. Thirdly, the Kolmogorov-Smirnov test was used to check whether difficulty scores of consistent and inconsistent objects are from the same distribution or not.
\end{description}
...
This section also presents examples of inconsistent span-tag pairs, which we identify as challenging or noisy in the context of multi-assessor annotations. These examples can be found in Tab.~\ref{tab:ma_diff}. In most cases of low confidence scores, a small number of assessors likely selected an incorrect tag (indicating noisy markups), or there was disagreement among assessors regarding tag selection (reflecting difficult objects). Additionally, a small number of assessors may have highlighted the span, which could signal either assessor error or the complexity of the task.

\begin{table}[t]
    \centering
    \caption{Difficult and noisy objects with low consistency scores}\label{tab:ma_diff}
    \begin{tabular}{|p{9.5cm}|c|}
    \hline
        Document with span & Tag and score \\ \hline
        \uwave{Leta Hong Fincher} has more. & per-0.5, org-0.5 \\ \hline
        Last month, Lebanese officials claimed victory in the fighting, but daily firefights have continued \uwave{since then}. & tim-0.33 \\ \hline
        \uwave{Bird flu} has killed at least 81 people in East Asia and Turkey since 2003. & nat-0.33 \\ \hline
        \uwave{Television} reports say few onlookers turned out for a glimpse of the Prince of Wales and his long-time companion, the Duchess of Cornwall, whom he married earlier this year. & org-0.5 \\ \hline
        Television reports say few onlookers turned out for a glimpse of the \uwave{Prince of Wales} and his long-time companion, the Duchess of Cornwall, whom he married earlier this year. & geo-0.5, per-0.5 \\ \hline
        Television reports say few onlookers turned out for a glimpse of the Prince of Wales and his long-time companion, the \uwave{Duchess of Cornwall}, whom he married earlier this year. & geo-0.5, per-0.5 \\ \hline
        \uwave{Muslim} separatists in Indian Kashmir are fighting for an independent Kashmir or its merger with neighbouring Pakistan. & org-0.5 \\ \hline
    \end{tabular}
\end{table}
...
\begin{table}[!htb]
    ...
    \begin{tabular}{|c|c|c|c|c|c|} 
        \hline
        \multicolumn{2}{|c|}{\textbf{Encoder and method}} & \textbf{Accuracy} & \textbf{Precision-Macro} & \textbf{Recall-Macro} & \textbf{F1-micro} \\ \hline
        \multirow{6}{*}{\rotatebox{90}{BERT}} & mean & 0.887 & 0.761 & 0.699 & 0.720 \\ \cline{2-6}	 
        & attention & 0.900 & 0.652 & 0.648 & 0.650 \\ \cline{2-6}
        & max & 0.914&0.717&0.647&0.667 \\ \cline{2-6}
        & endpoint & 0.915&0.750&0.663&0.689\\ \cline{2-6}
        & diff-sum &0.914&0.761&0.701&0.724\\ \cline{2-6}
        & coherent &0.914&0.788&0.702&0.732\\ \hline
        \multirow{6}{*}{\rotatebox{90}{DeBERTa}} & mean &0.899&0.705&0.640&0.657\\ \cline{2-6}
        & attention &0.898&0.704&0.614&0.624\\ \cline{2-6}
        & max &0.915&0.716&0.646&0.666\\ \cline{2-6}
        & endpoint &0.917&0.810&0.704&0.739\\ \cline{2-6}
        & diff-sum &$\bm{0.919}$&$\bm{0.812}$&$\bm{0.705}$&$\bm{0.740}$\\ \cline{2-6}
        & coherent &0.916&0.701&0.656&0.671\\ \hline
...
\begin{table}[!htb]
    \centering
    \caption{ROC-AUC for difficulty assessing methods applied to different difficult datasets (SB is the abbreviation of SpanBERT in this table)}\label{tab:diff_val}
    {
    \tiny
    \begin{tabular}{|c|*{13}{c|}}
    \hline
    \multicolumn{2}{|c|}{\multirow{2}{*}{\textbf{\shortstack{Encoder and\\ method}}}} & \multicolumn{4}{c|}{\textbf{Noisy dataset}} & \multicolumn{4}{c|}{\textbf{Drug-dosage}} & \multicolumn{4}{c|}{\textbf{Drug-effect}} \\ \cline{3-14}
    \multicolumn{2}{|c|}{} & VoG & $-M(x)$ & $-|M(x)|$ & Gaus. & VoG & $-M(x)$ & $-|M(x)|$ & Gaus. & VoG & $-M(x)$ & $-|M(x)|$ & Gaus. \\ \hline
    \multirow{6}{*}{\rotatebox{90}{BERT}} & mean & 0.563 & 0.584 & 0.590 & 0.999 & 0.737 & 0.766 & 0.802 & 0.948 & 0.694 & 0.698 & 0.732 & 0.933 \\ \cline{2-14}
    & attention & 0.542 & 0.571 & 0.573 & 0.999 & 0.796 & 0.830 & $\bm{0.846}$ & 0.953 & 0.758 & 0.783 & 0.801 & 0.936 \\ \cline{2-14}
    & max & 0.560 & 0.594 & 0.593 & 0.456 & 0.855 & 0.826 & 0.820 & 0.958 & 0.863 & 0.841 & $\bm{0.828}$ & 0.950 \\ \cline{2-14}
    & endpoint & 0.581 & 0.610 & 0.610 & $\bm{1.000}$ & 0.826 & 0.830 & 0.801 & 0.960 & 0.842 & 0.788 & 0.764 & 0.957 \\ \cline{2-14}
    & diff-sum & 0.571 & 0.611 & 0.610 & $\bm{1.000}$ & $\bm{0.869}$ & $\bm{0.865}$ & 0.805 & $\bm{0.967}$ & 0.856 & $\bm{0.863}$ & 0.808 & $\bm{0.965}$ \\ \cline{2-14}
    & coherent & 0.546 & 0.605 & 0.603 & 0.472 & 0.848 & 0.845 & 0.806 & 0.935 & $\bm{0.867}$ & 0.852 & 0.810 & 0.925 \\ \hline
    \multirow{6}{*}{\rotatebox{90}{DeBERTa}} & mean & 0.578 & 0.598 & 0.599 & 0.999 & 0.762 & 0.742 & 0.779 & 0.935 & 0.767 & 0.683 & 0.716 & 0.925 \\ \cline{2-14}
    & attention & $\bm{0.617}$ & 0.603 & 0.606 & 0.999 & 0.572 & 0.818 & 0.842 & 0.945 & 0.539 & 0.759 & 0.769 & 0.921 \\ \cline{2-14}
    & max & 0.572 & 0.613 & 0.613 & 0.424 & 0.839 & 0.807 & 0.796 & 0.910 & 0.846 & 0.780 & 0.751 & 0.891 \\ \cline{2-14}
    & endpoint & 0.553 & 0.616 & 0.616 & $\bm{1.000}$ & 0.800 & 0.831 & 0.767 & 0.941 & 0.827 & 0.837 & 0.752 & 0.937 \\ \cline{2-14}
    & diff-sum & 0.580 & $\bm{0.616}$ & $\bm{0.616}$ & $\bm{1.000}$ & 0.845 & 0.809 & 0.781 & 0.913 & 0.835 & 0.831 & 0.789 & 0.920 \\ \cline{2-14}
    & coherent & 0.571 & 0.609 & 0.608 & 0.448 & 0.805 & 0.830 & 0.712 & 0.900 & 0.801 & 0.815 & 0.685 & 0.873 \\ \hline
    SB & endpoint & 0.560 & 0.612 & 0.611 & $\bm{1.000}$ & 0.841 & 0.848 & 0.725 & 0.955 & 0.864 & 0.846 & 0.695 & 0.946 \\ \hline
    \end{tabular}
    }
\end{table}

\subsection{Correlations between difficulty assessing methods}

In this experiment Spearman's rank correlation was calculated for each pair of difficulty assessing methods for each learned model. We considered only difficulty scores on objects from validation set $D_{val}$. These correlations are provided in Table~\ref{tab:diff_corr}.
...
\begin{table}[!htb]
    \centering
    \caption{Correlations between different difficulty assessing methods for different models (SB is the abbreviation of SpanBERT in this table)}\label{tab:diff_corr}
    {
    \tiny
    \begin{tabular}{|c|*{13}{c|}}
    \hline
    \multicolumn{2}{|c|}{\multirow{2}{*}{\textbf{\shortstack{Encoder and\\ method}}}} & \multicolumn{3}{c|}{\textbf{VoG}} & \multicolumn{3}{c|}{\textbf{$-M(x)$}} & \multicolumn{3}{c|}{\textbf{$-|M(x)|$}} & \multicolumn{3}{c|}{\textbf{Gaussian}}\\ \cline{3-14}
    \multicolumn{2}{|c|}{} & $-M(x)$ & $-|M(x)|$ & Gaus. & VoG & $-|M(x)|$ & Gaus. & VoG & $-M(x)$ & Gaus. & VoG & $-M(x)$ & $-|M(x)|$\\ \hline
    \multirow{6}{*}{\rotatebox{90}{BERT}} & mean & $\bm{0.886}$&$\bm{0.835}$&$\bm{0.621}$&$\bm{0.886}$&0.936&0.560&$\bm{0.835}$&0.936&0.521&$\bm{0.621}$&0.560&0.521 \\ \cline{2-14}
    & attention & 0.829&0.789&0.607&0.829&0.944&0.602&0.789&0.944&0.592&0.607&0.602&0.592 \\ \cline{2-14}
    & max & 0.725&0.689&0.526&0.725&0.958&0.398&0.689&0.958&0.391&0.526&0.398&0.391\\ \cline{2-14}
    & endpoint & 0.739&0.701&0.476&0.739&0.963&0.314&0.701&0.963&0.302&0.476&0.314&0.302 \\ \cline{2-14}
    & diff-sum & 0.718&0.679&0.538&0.718&0.963&0.289&0.679&0.963&0.278&0.538&0.289&0.278 \\ \cline{2-14}
    & coherent & 0.702&0.670&0.600&0.702&0.964&0.428&0.670&0.964&0.422&0.600&0.428&0.422\\ \hline
    \multirow{6}{*}{\rotatebox{90}{DeBERTa}} & mean & 0.806&0.762&0.226&0.806&0.940&0.238&0.762&0.940&0.216&0.226&0.238&0.216 \\ \cline{2-14}
    & attention & 0.575&0.490&0.142&0.575&0.936&$\bm{0.604}$&0.490&0.936&$\bm{0.602}$&0.142&$\bm{0.604}$&$\bm{0.602}$ \\ \cline{2-14}
    & max & 0.662&0.633&0.276&0.662&0.962&0.062&0.633&0.962&0.054&0.276&0.062&0.054 \\ \cline{2-14}
    & endpoint & 0.589&0.557&0.474&0.589&0.968&0.186&0.557&0.968&0.180&0.474&0.186&0.180 \\ \cline{2-14}
    & diff-sum & 0.727&0.697&0.409&0.727&0.968&0.081&0.697&0.968&0.071&0.409&0.081&0.071 \\ \cline{2-14}
    & coherent & 0.685&0.665&0.386&0.685&$\bm{0.970}$&0.211&0.665&$\bm{0.970}$&0.212&0.386&0.211&0.212 \\ \hline
    SB & endpoint & 0.676&0.644&0.398&0.676&0.965&0.203&0.644&0.965&0.196&0.398&0.203&0.196 \\ \hline
    \end{tabular}
    }
\end{table}

\subsection{Interpretability of methods assessing difficulty}
...
\begin{table}[!htb]
    ...
    \begin{tabular}{|c|c|c|c|c|c|} 
        \hline
        \multicolumn{2}{|c|}{\textbf{Encoder and method}} & VoG & $-M(x)$ & $-|M(x)|$ & \textbf{Gaussian} \\ \hline
        \multirow{6}{*}{\rotatebox{90}{BERT}} & mean & 0.000 & 0.013 & 0.009 & 0.014 \\ \cline{2-6} 
        & attention & 0.004 & 0.010 & 0.004 & $\bm{0.015}$ \\ \cline{2-6}
        & max & 0.003&$\bm{0.028}$&$\bm{0.027}$&0.008 \\ \cline{2-6}
        & endpoint & 0.001&0.028&0.026&0.002\\ \cline{2-6}
        & diff-sum &0.002&0.028&0.025&0.002\\ \cline{2-6}
        & coherent &0.000&0.028&0.026&0.014\\ \hline
        \multirow{6}{*}{\rotatebox{90}{DeBERTa}} & mean &0.004&0.010&0.003&0.015\\ \cline{2-6}
        & attention &$\bm{0.016}$&0.008&0.004&0.007\\ \cline{2-6}
        & max &-0.003&0.012&0.009&0.007\\ \cline{2-6}
        & endpoint &-0.003&0.020&0.016&0.001\\ \cline{2-6}
        & diff-sum &0.004&0.021&0.017&0.003\\ \cline{2-6}
        & coherent &-0.001&0.020&0.016&0.000\\ \hline
...
\begin{table}[!htb]
    ...
    \begin{tabular}{|c|c|c|c|c|c|} 
        \hline
        \multicolumn{2}{|c|}{\textbf{Encoder and method}} & VoG & $-M(x)$ & $-|M(x)|$ & \textbf{Gaussian} \\ \hline
        \multirow{6}{*}{\rotatebox{90}{BERT}} & mean & 0.503 & 0.628 & 0.581 & 0.634 \\ \cline{2-6} 
        & attention & 0.458 & 0.599 & 0.541 & 0.641 \\ \cline{2-6}
        & max & 0.530&$\bm{0.772}$&$\bm{0.755}$&0.573 \\ \cline{2-6}
        & endpoint & 0.508&0.765&0.746&0.479\\ \cline{2-6}
        & diff-sum &0.484&0.766&0.743&0.478\\ \cline{2-6}
        & coherent &0.499&0.765&0.746&0.630\\ \hline
        \multirow{6}{*}{\rotatebox{90}{DeBERTa}} & mean &0.535&0.596&0.525&$\bm{0.647}$\\ \cline{2-6}
        & attention &0.351&0.578&0.538&0.435\\ \cline{2-6}
        & max &0.472&0.613&0.589&0.439\\ \cline{2-6}
        & endpoint &0.473&0.688&0.649&0.507\\ \cline{2-6}
        & diff-sum &0.542&0.706&0.660&0.475\\ \cline{2-6}
        & coherent &0.492&0.693&0.655&0.504\\ \hline
...
\begin{table}[!htb]
    ...
    \begin{tabular}{|c|c|c|c|c|c|} 
        \hline
        \multicolumn{2}{|c|}{\textbf{Encoder and method}} & VoG & $-M(x)$ & $-|M(x)|$ & \textbf{Gaussian} \\ \hline
        \multirow{6}{*}{\rotatebox{90}{BERT}} & mean & 0.940 & 0.399 & 0.355 & 0.386 \\ \cline{2-6}	 
        & attention & 0.960 & 0.307 & 0.264 & 0.224 \\ \cline{2-6}
        & max & 0.838&$\bm{0.012}$&$\bm{0.009}$&0.392 \\ \cline{2-6}
        & endpoint & 0.837&0.028&0.024&0.677\\ \cline{2-6}
        & diff-sum &0.865&0.049&0.044&0.835\\ \cline{2-6}
        & coherent &0.939&0.025&0.021&$\bm{0.128}$\\ \hline
        \multirow{6}{*}{\rotatebox{90}{DeBERTa}} & mean &0.900&0.822&0.783&0.389\\ \cline{2-6}
        & attention &0.120&0.606&0.506&0.456\\ \cline{2-6}
        & max &0.702&0.736&0.703&0.869\\ \cline{2-6}
        & endpoint &0.751&0.272&0.447&0.861\\ \cline{2-6}
        & diff-sum &0.718&0.132&0.243&0.783\\ \cline{2-6}
        & coherent &0.781&0.415&0.404&0.966\\ \hline
        SpanBERT & endpoint &$\bm{0.113}$&0.125&0.089&0.576\\ \hline
    \end{tabular}
\end{table}

\section{Conclusion}
This article presents a pipeline for estimating the model-specific difficulty of a tagged text span, with an ultimate goal of enhancing the performance of machine learning models. Within this context, the difficulty of an object is defined as the complexity a machine learning model encounters when processing it for a specific task. An established language model architecture, recognised for its high-quality vectorisation capabilities, was adapted for this purpose. A language model was subsequently trained and evaluated using our approach. The efficacy of the task-specific difficulty assessment methods was then analysed through multiple approaches; these included calculating the ROC-AUC metric across various pseudo-difficult datasets and conducting a qualitative examination of objects assigned the highest difficulty scores.

The Gaussian approach~\cite{Lee2018ASU} was found to achieve the highest ROC-AUC score. In contrast, the margin method, which utilises absolute values to derive difficulty scores, demonstrated greater interpretability. Furthermore, our analysis identified variations in the perceived difficulty of individual objects, which were contingent upon the specific assessment method employed.

Besides that, DeBERTa with endpoint and diff-sum span aggregations achived the highest ROC-AUC scores across all methods which indicates a connection between the quality of models and the quality of the difficulty assessment of spans relative to its work.
